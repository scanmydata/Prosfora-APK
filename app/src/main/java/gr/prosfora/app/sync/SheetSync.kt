...
class SheetSync(
    ...
    suspend fun sync(): Report = withContext(Dispatchers.IO) {
        ...
        // Δεν πειράζουμε το TAB_EMPLOYEE_COSTS εδώ. Είναι παράγωγα δεδομένα από OCR
        // και διατηρούνται ανεξάρτητα ώστε να μη γίνεται ξανά υπολογισμός κάθε sync.
    ...
    private fun announceForeignDebts(local: List<DebtEntity>, merged: List<DebtEntity>) {
        ...
        val fresh = merged.filter { debt ->
            !debt.deleted && debt.id !in known && debt.createdBy.isNotBlank() &&
                debt.createdBy.trim().lowercase() != mine
        }
        if (fresh.isNotEmpty()) DriveNotifier.notifyDebts(context, fresh)
    }

    suspend fun createSharedSheet(title: String, drive: gr.prosfora.app.google.DriveClient? = null): String =
    ...
    private suspend fun ensureTabs(spreadsheetId: String) {
        ...
        if (TAB_EMPLOYEE_COSTS !in existing) {
            ...
            val rows = sheets.readRows(spreadsheetId, TAB_EMPLOYEE_COSTS)
            if (rows.isEmpty()) sheets.replaceRows(spreadsheetId, TAB_EMPLOYEE_COSTS, listOf(EMPLOYEE_COST_HEADER))
        }
    }
    ...
    private suspend fun readOffers(spreadsheetId: String): List<OfferEntity> =
        dataRows(spreadsheetId, TAB_OFFERS, OFFER_HEADER.size).map { row ->
            OfferEntity(
                id = row[0], address = row[1], dateEpochDay = row[2].toLongOrNull() ?: 0L,
                kind = row[3], email = row[4],
                status = runCatching { OfferStatus.valueOf(row[5]) }.getOrElse { OfferStatus.fromLabel(row[5]) },
                createdAt = row[6].toLongOrNull() ?: 0L, updatedAt = row[7].toLongOrNull() ?: 0L,
                lastSentAt = row[8].toLongOrNull(), deleted = row[9] == "1", customerName = row[10],
                customerPhone = row[11], notifiedAt = row[12].toLongOrNull(),
                notifiedVia = row[13].ifBlank { null }, workStartDay = row[14].toLongOrNull(),
                workEndDay = row[15].toLongOrNull(), reviewSentAt = row[16].toLongOrNull(),
                validUntilDay = row[17].toLongOrNull(), paymentTerms = row[18], source = row[19],
                customerLastName = row[20], vatIncluded = row[22] == "1", scaffolding = row[23] == "1",
                scaffoldingCost = row[24].toDoubleOrNull() ?: 0.0, permit = row[25] == "1",
    ...
    private suspend fun readDebts(spreadsheetId: String): List<DebtEntity> =
        dataRows(spreadsheetId, TAB_DEBTS, DEBT_HEADER.size).map { row ->
            DebtEntity(
                id = row[0], kind = runCatching { DebtKind.valueOf(row[1]) }.getOrDefault(DebtKind.AADE),
                periodMonth = row[2].toIntOrNull() ?: 0, periodYear = row[3].toIntOrNull() ?: 0,
                dueDay = row[4].toLongOrNull(), amount = row[5].toDoubleOrNull() ?: 0.0,
                reference = row[6], description = row[7], personName = row[8], personCode = row[9],
                paid = row[10] == "1", paidAt = row[11].toLongOrNull(), paidDay = row[17].toLongOrNull(),
                createdBy = row[18], source = row[12], driveFileId = row[13],
                createdAt = row[14].toLongOrNull() ?: 0L, updatedAt = row[15].toLongOrNull() ?: 0L,
                deleted = row[16] == "1",
            )
        }
    ...
    private suspend fun readEmployees(spreadsheetId: String): List<EmployeeEntity> =
        dataRows(spreadsheetId, TAB_PEOPLE, PEOPLE_HEADER.size).map { row ->
            EmployeeEntity(
                id = row[0], name = row[1], alias = row[2], code = row[3],
    ...
    private suspend fun readSpaces(spreadsheetId: String): List<SpaceEntity> =
        dataRows(spreadsheetId, TAB_SPACES, SPACE_HEADER.size).map { row ->
            SpaceEntity(
                id = row[0], offerId = row[1], description = row[2], area = row[3].toDoubleOrNull() ?: 0.0,
                unitPrice = row[4].toDoubleOrNull() ?: 0.0, position = row[5].toIntOrNull() ?: 0,
                updatedAt = row[6].toLongOrNull() ?: 0L, deleted = row[7] == "1",
            )
        }
    ...
    private suspend fun readNotes(spreadsheetId: String): List<NoteEntity> =
        dataRows(spreadsheetId, TAB_NOTES, NOTE_HEADER.size).map { row ->
            NoteEntity(
                id = row[0], offerId = row[1], text = row[2], position = row[3].toIntOrNull() ?: 0,
                updatedAt = row[4].toLongOrNull() ?: 0L, deleted = row[5] == "1",
            )
        }
    ...
    private suspend fun dataRows(spreadsheetId: String, tab: String, width: Int): List<List<String>> {
        ...
        return rows.drop(1).filter { it.firstOrNull()?.isNotBlank() == true }
            .map { row -> List(width) { i -> row.getOrElse(i) { "" } } }
    }

    private fun offerRows(offers: List<OfferEntity>) = listOf(OFFER_HEADER) + offers.map {
        listOf(
            it.id, it.address, it.dateEpochDay.toString(), it.kind, it.email, it.status.name, it.createdAt.toString(),
            it.updatedAt.toString(), it.lastSentAt?.toString().orEmpty(), if (it.deleted) "1" else "0", it.customerName,
            it.customerPhone, it.notifiedAt?.toString().orEmpty(), it.notifiedVia.orEmpty(), it.workStartDay?.toString().orEmpty(),
            it.workEndDay?.toString().orEmpty(), it.reviewSentAt?.toString().orEmpty(), it.validUntilDay?.toString().orEmpty(),
            it.paymentTerms, it.source, it.customerLastName, it.customerGender.name, if (it.vatIncluded) "1" else "0",
            if (it.scaffolding) "1" else "0", it.scaffoldingCost.toString(), if (it.permit) "1" else "0", it.permitCost.toString(),
        )
    }

    private fun debtRows(debts: List<DebtEntity>) = listOf(DEBT_HEADER) + debts.map {
        listOf(
            it.id, it.kind.name, it.periodMonth.toString(), it.periodYear.toString(), it.dueDay?.toString().orEmpty(),
            it.amount.toString(), it.reference, it.description, it.personName, it.personCode, if (it.paid) "1" else "0",
            it.paidAt?.toString().orEmpty(), it.source, it.driveFileId, it.createdAt.toString(), it.updatedAt.toString(),
            if (it.deleted) "1" else "0", it.paidDay?.toString().orEmpty(), it.createdBy,
        )
    }

    private fun employeeRows(people: List<EmployeeEntity>) = listOf(PEOPLE_HEADER) + people.map {
        listOf(it.id, it.name, it.alias, it.code, it.updatedAt.toString(), if (it.deleted) "1" else "0", it.leftDay?.toString().orEmpty())
    }
    ...
    companion object {
        ...
        val EMPLOYEE_COST_HEADER = listOf(
            "ID_Εργαζομένου", "Όνομα", "Έτος", "Μήνας", "Πληρωτέο", "Κόστος ενσήμων", "Αρχείο Drive", "Ενημερώθηκε",
        )

        private val OFFER_HEADER = listOf(
            "ID_Προσφοράς", "Οδός / Περιοχή", "Ημερομηνία", "Είδος", "Email", "Κατάσταση", "Δημιουργήθηκε", "Ενημερώθηκε", "Στάλθηκε", "Διαγραμμένο",
            "Ονοματεπώνυμο", "Κινητό", "Ειδοποιήθηκε", "Μέσο ειδοποίησης", "Έναρξη εργασιών", "Ολοκλήρωση εργασιών", "Αξιολόγηση", "Ισχύει έως",
            "Τρόπος πληρωμής", "Πηγή", "Επώνυμο", "Φύλο", "ΦΠΑ", "Σκαλωσιά", "Κόστος σκαλωσιάς", "Άδεια", "Κόστος άδειας",
        )
        private val DEBT_HEADER = listOf(
            "ID_Οφειλής", "Φορέας", "Μήνας", "Έτος", "Λήξη", "Ποσό", "Ταυτότητα / RF", "Περιγραφή", "Εργαζόμενος", "Κωδικός",
            "Πληρώθηκε", "Ημ. πληρωμής", "Πηγή", "Αρχείο Drive", "Δημιουργήθηκε", "Ενημερώθηκε", "Διαγραμμένο", "Ημ. εξόφλησης", "Καταχωρήθηκε από",
        )
